/**
 * Plantilla PDF: Informe Ejecutivo de Auditabilidad.
 * Genera el informe de auditoría con datos generales, matriz de riesgo,
 * estado por bloque legal, hallazgos críticos, plan CAPA y anexo del checklist.
 */
...
const titulo = (doc, texto) => {
  doc.fillColor('#111827').fontSize(13).font('Helvetica-Bold').text(texto);
  doc.moveDown(0.3);
...
const construirInformeAuditoria = (auditoria, resumen) => {
  const doc = new PDFDocument({
    size: 'A4',
    margin: 40,
    info: { Title: `Informe de auditoría ${auditoria.codigo || ''}` },
  });

  doc.fillColor('#0f172a').fontSize(17).font('Helvetica-Bold').text('Informe Ejecutivo de Auditabilidad');
  ...
  parrafoDato(doc, '% Cumplimiento', `${resumen.porcentajeCumplimiento}%`);
  parrafoDato(doc, '% No cumplimiento', `${resumen.porcentajeNoCumplimiento}%`);

  doc.moveDown(0.3);
  doc.font('Helvetica-Bold').fillColor(COLORES[resumen.nivelRiesgo] || '#111827').fontSize(12)
  ...
  if (!resumen.hallazgosCriticos.length) {
    ...
    resumen.hallazgosCriticos.forEach((hallazgo) => {
      doc.fillColor('#b91c1c').font('Helvetica-Bold').text(`${hallazgo.codigo} (${hallazgo.bloque})`);
      doc.fillColor('#111827').font('Helvetica').text(hallazgo.requisito);
      if (hallazgo.observaciones) {
        doc.fillColor('#475569').text(`Observación: ${hallazgo.observaciones}`).fillColor('#111827');
      }
      doc.moveDown(0.2);
    });
  }
  ...
  const noCumple = (auditoria.items || []).filter((item) => item.estado === 'no_cumple');
  titulo(doc, 'Plan de acciones correctivas y preventivas (CAPA)');
  if (!noCumple.length) {
    doc.text('Sin incumplimientos registrados: no se requieren acciones correctivas.');
  } else {
    noCumple.forEach((item) => {
      if (doc.y > 720) doc.addPage();
      doc.font('Helvetica-Bold').text(`${item.requisito?.codigo} - ${item.requisito?.bloque}`);
      doc.font('Helvetica').text(item.requisito?.requisito || '');
      if (item.observaciones) doc.fillColor('#475569').text(`Hallazgo: ${item.observaciones}`);
      doc.fillColor('#111827').text(`Acción correctiva: ${item.accionCorrectiva || 'Por definir'}`);
      doc.text(`Responsable: ${item.responsableAccion || 'Por asignar'} | Fecha compromiso: ${fecha(item.fechaCompromiso)}`);
      doc.moveDown(0.4);
    });
  }
  ...
  (auditoria.items || []).forEach((item) => {
    ...
    doc.font('Helvetica-Bold').fontSize(9).text(
      `${item.requisito?.codigo} [${ESTADO_LABEL[item.estado] || 'Sin evaluar'}]${item.requisito?.critico ? ' *CRÍTICO*' : ''}`
    );
    doc.font('Helvetica').fontSize(9).text(item.requisito?.requisito || '');
    if (item.observaciones) doc.fillColor('#475569').text(`Obs: ${item.observaciones}`).fillColor('#111827');
    doc.moveDown(0.25);
  });
...
export { construirInformeAuditoria };
